...
const MonthlyList = ({ totalCount, thisYearMonthlyList }) => {
  ...
  const currentMonthCount = thisYearMonthlyList[new Date().getMonth()]?.count ?? 0;

  const infoList = [
    { label: "累計", value: totalCount, testId: "totalCount" },
    { label: "今年", value: annualCount, testId: "annualCount" },
    { label: "今月", value: currentMonthCount, testId: "currentMonthCount" }
  ];

  return (
    <List>
      <Divider />
      {infoList.map(({ label, value, testId }) => {
        return (
          <>
            <ListItem sx={{ display: 'flex', justifyContent: 'space-between', overflow: 'hidden' }} key={label}>
              ...
              <Typography variant="body1" sx={{ color: '#000' }} data-testid={testId}>{value}冊</Typography>
            </ListItem>
            <Divider />
          </>
        );
